...
const CommentInput = ({ noteId, onAddComment }) => {
  const [commentText, setCommentText] = useState("");

  const handleSubmit = () => {
    if (commentText.trim()) {
      onAddComment(noteId, commentText);
      setCommentText("");
    }
  };
          ...
          <div className="mt-4 pt-4 border-t border-gray-100">
            <div className="space-y-3">
              {noteComments
                ...
                .slice(0, showAllComments[note._id] ? undefined : 1)
                .map((comment, index) => (
              ...
              {noteComments.length > 1 && (
                ...
                >
                  {showAllComments[note._id]
                    ? "Show less"
                    : `Show ${noteComments.length - 1} more comments`}
                </button>
              )}
              ...
              {isLoggedIn && (
                <CommentInput
                  noteId={note._id}
                  onAddComment={async (noteId, text) => {
                    try {
                      const { data: newC } = await createComment(noteId, text);
                      newC.user = {
                        name:
